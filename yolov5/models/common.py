...
class ASPP(nn.Module):
    # Atrous Spatial Pyramid Pooling (ASPP) layer for DeepLabV3 <https://arxiv.org/abs/1706.05587>
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 4  # hidden channels
        self.cv1 = Conv(c1, c_, 3, 1)
        self.cv2 = Conv(c1, c_, 3, 1, d=2)
        self.cv3 = Conv(c1, c_, 3, 1, d=3)
        self.cv4 = Conv(c1, c_, 3, 1, d=4)
        
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        
        self.cv = nn.Conv2d(c_*8, c2, 1, bias=False)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')  # suppress torch 1.9.0 max_pool2d() warning
            y1 = self.m(x)
            return self.cv(torch.cat((x1, x2, x3, x4, y1), 1))
